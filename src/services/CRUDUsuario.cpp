...
bool CRUDUsuario::cadastrar(const string& nome, const string& cpf, const string& telefone, const string& email, const string& dataCadastro) {
    ...
    if (sucesso) cout << "Usuario cadastrado com sucesso!" << endl;
    else cout << "Erro ao cadastrar usuario: " << db.getMensagemErro() << endl;
...
bool CRUDUsuario::buscarPorId(int id) {
    ...
    if (contador == 0) cout << "Nenhum usuario encontrado com ID: " << id << endl;
    return contador > 0;
}
...
bool CRUDUsuario::listarTodos() {
    ...
    db.consultar(
        ...
        [&](const Database::Linha& linha) {
            contador++;
            cout << "ID: " << linha.at("id") << " | Nome: " << linha.at("nome") << endl;
        }
    );
    
    if (contador == 0) cout << "Nenhum usuario cadastrado." << endl;
    return contador > 0;
}
...
bool CRUDUsuario::atualizar(int id, const string& novoTelefone, const string& novoEmail) {
    ...
    if (sucesso) cout << "Usuario atualizado com sucesso!" << endl;
    else cout << "Erro ao atualizar usuario: " << db.getMensagemErro() << endl;
...
bool CRUDUsuario::deletar(int id) {
    ...
    if (sucesso) cout << "Usuario removido com sucesso!" << endl;
    else cout << "Erro ao remover usuario: " << db.getMensagemErro() << endl;
